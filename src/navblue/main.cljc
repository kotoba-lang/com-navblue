(ns navblue.main
  "Kotodama WASM entrypoint for the navblue clean-room actor (L4) — Clojure port.
  ...
  facts (`navblue.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "navblue")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"navblue.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "navblue-compat" :tier tier :entities entities} 200])
...
(defn start! [] :navblue-compat/ready)
